...
每个拉取请求和默认分支推送都会检出完整 Git 历史，运行无依赖的集成偏差检查，安装锁定的 Ruby 与 Node 依赖，并对配置的宿主内容执行真实的生产构建。该构建会编译前端资源、审计生成站点并校验 URL。

工作流会检查整份拉取请求差异，而不是只看最近一次推送，并据此选择两档验证之一：

- **宿主验证**适用于内容、翻译和宿主配置变更。它保留生产宿主构建及其审计，但跳过 Chromium、模板与浏览器回归、视觉基线和示例矩阵。
- **完整回归**适用于 Jekyll Obsidian 实现变更。它会安装 Chromium，以浏览器覆盖运行 `website/bin/test`，并构建全部示例部署形态。项目自带的示例会分别在域名根路径构建两个主题，并在项目路径下再次构建 Minimal：

1. 域名根路径，例如 `https://owner.github.io/`。
2. 项目路径，例如 `https://owner.github.io/jekyll-obsidian/`。

`website/` 下的任何变更都会选择完整回归，唯一例外是源仓库自身的内容目录 `website/docs/**`。生成的 `.github/workflows/pages.yml` 一旦变化，也会选择完整回归。同一拉取请求只要同时包含内容和实现变更，就始终执行完整回归。手动运行、首次推送、无效 commit ID、历史缺失或 Git 比较失败都会安全回退到完整回归；工作流不会复用旧运行的分类结果。

拉取请求作业只有 `contents: read` 权限。它们不会调用 `configure-pages`、上传 Pages 构建产物或执行部署。

工作流会监视配置的内容目录、`website/**`、`.github/jekyll-obsidian.yml` 和自身文件。不要手工编辑这些触发路径；内容目录更改后，请重新运行集成命令。完整回归中的浏览器测试与视觉基线始终使用 `website/docs`；两档验证都会通过独立生产构建校验宿主内容，因此自定义文档不会被误判为视觉回归。默认分支上的纯内容推送通过验证后，仍会继续执行受信任的 Pages 构建与部署。
