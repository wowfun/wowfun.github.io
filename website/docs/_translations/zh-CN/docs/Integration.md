...
未提供 `--to` 时，命令会从不可变的附注标签（annotated tag）`vX.Y.Z` 中选择最新的官方稳定版本。`X.Y.Z` 是语义化版本（Semantic Versioning）的核心版本：每个数字标识符要么是 `0`，要么以 `1` 至 `9` 开头。公开接口仍处于初始开发阶段时，版本使用 `0.y.z`。发布日期写入发行说明，不写进版本号。目标可以是当前已安装版本，也可以向前更新，但不能降级。`--check` 不写文件。安装已经是目标版本时返回 `0`，存在更新或可首次记录来源时返回 `2`，参数无效、本地偏离、网络失败或发行版本无效时返回 `1`。

`--to` 接受不含前缀 `v` 的官方稳定核心版本；上面的 `0.1.0` 只用于展示格式。带预发布标识符或构建元数据的标签，例如 `v0.2.0-rc.1` 或 `v0.2.0+sha.abcdef`，不属于官方稳定更新通道。更新器会拒绝把它们作为目标，默认选择时也会忽略。没有稳定版本时，命令会报告该状态，并保持宿主不变。安装必须与某个官方标签匹配，严格更新才能建立或验证其来源。
...
没有来源锁的安装只有在已提交 `website/` 的路径与内容集合完全匹配其内置版本对应的官方标签时，才能被接管。未带标签的检出无法证明来源。没有匹配标签时，请先用完整的带标签快照替换一次 `website/`，重新运行 `integrate`，然后再次运行 `update`。发行标签不可变；已安装标签后续被删除或移动都会被视为错误。
